package com.hansung.main.ch9;

import jakarta.persistence.EntityManager;
import jakarta.persistence.EntityTransaction;
import com.hansung.jpa.EMF;
import com.hansung.domain.Question;
...
public class MainQuestion1 {
    ...
    private static void readQuestion(String id) {
        ...
        try {
            tx.begin();
            // set 매핑과 마찬가리조 lazy냐 eager이냐에 따라 question_choice의 내용을 나중에 가져오거나 left join으로 같이 가져옴
    ...
    private static void removeQuestion(String id) {
        ...
        try {
            tx.begin();
            Question q = em.find(Question.class, id);
            // 엔티티 삭제시 컬렉션에 있는 값도 같이 삭제 (question_choice, question 둘다 삭제)
            em.remove(q);
            tx.commit();
        } catch (Exception e) {
            tx.rollback();
